        ...
        match self.log_file_path {
            Some(ref log_file_path) => {
                ...
                if !path.exists() || path.is_file() {
                    Ok(path.canonicalize().expect("Failed to canonicalize log file path"))
                }
                else {
...
mod tests {
    ...
    fn it_works() {
        let state = 
            PersistentStateBuilder::default()
                .current_term(10)
    ...
    fn save_to_disk_works() {
        ...
        let raft = 
            ...
            .metadata(NodeMetadataBuilder::default().log_file_path(PathBuf::from("/tmp/raftd.conf")).build().expect("Couldn't build metadata with builder."))
            .build()
