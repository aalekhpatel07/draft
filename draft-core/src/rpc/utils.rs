...
#[allow(dead_code)]
pub(crate) fn vote_request(term: usize, candidate_id: usize, last_log_index: usize, last_log_term: usize) -> VoteRequest {
...
pub(crate) fn append_entries_request(
    ...
    AppendEntriesRequest { term, leader_id, previous_log_index, previous_log_term, entries: log, leader_commit_index }
}

#[allow(dead_code)]
pub(crate) fn persistent_state(current_term: usize, voted_for: Option<usize>, terms: Vec<usize>) -> PersistentState
{
    let mut log = Vec::new();
    log.extend(terms.iter().map(|term| (*term, Bytes::from(""))));
