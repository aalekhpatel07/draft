...
**This implementation of mini-redis from Tokio has been adapted to be used as an underlying state machine with the Raft protocol. The original API provided by mini-redis may not be 100% consistent with this adaptation so it is unsafe to use this crate outside of the `draft-*` workspace.**
